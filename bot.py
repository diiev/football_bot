...
def main():
    ...
    entry_points=[MessageHandler(filters.Regex(r"^(5v5|6v6|8v8)$"), handle_generate_teams)],
    states={BALANCE_THRESHOLD: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_balance_threshold)]},
    fallbacks=[CommandHandler("cancel", cancel)]
    ))
    app.add_handler(MessageHandler(
    filters.TEXT & ~filters.COMMAND & ~filters.Regex(r"^\d+$"),  # Игнорируем числа
    handle_message
))
